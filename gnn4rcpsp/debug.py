...
def exploring(bench_id, data, result_file):
    ...
    for data in data_list:
        ...
        data.to(device)
        out = model(data)
        xorig = np.around(out[len(rc) :, 0].cpu().detach().numpy(), decimals=0).astype(
            int
        )
        ...
        perm = [
            model_rcpsp.index_task_non_dummy[t]
            for t in tasks
            if t in model_rcpsp.index_task_non_dummy
        ]
        sol = RCPSPSolution(problem=model_rcpsp, rcpsp_permutation=perm)
        print("SGS postpro : ", model_rcpsp.evaluate(sol))

        model_rcpsp.tasks_list = (
            [-1] + model_rcpsp.tasks_list[:30] + [150] + model_rcpsp.tasks_list[30:]
        )
        model_rcpsp.mode_details.pop(0)
        model_rcpsp.tasks_list.remove(0)
        model_rcpsp.mode_details[150] = {
            1: {
                k: 1 if k == "duration" else 0
                for k in ["duration"] + model_rcpsp.resources_list
            }
        }
        model_rcpsp.mode_details[-1] = {
            1: {k: 0 for k in ["duration"] + model_rcpsp.resources_list}
        }
        model_rcpsp.successors[150] = [
            model_rcpsp.sink_task,
            model_rcpsp.tasks_list[32],
        ]  # [model_rcpsp.source_task] # you can try uncommenting
        ...
        model_rcpsp = RCPSPModel(
            resources=model_rcpsp.resources,
...
def test(test_loader, test_list, model, device, result_file):
    ...
    for batch_idx, data in enumerate(
        ...
        json.dump(
            merged_res, open("res_with_cp/results_" + run_id + ".json", "w"), indent=4
        )
...
def modif_model():
    ...
    for delta in range(1, 10):
        ...
        for j in range(20):
            ...
            out = model(data)
            xorig = np.around(
                out[len(origin_rcpsp.resources_list) :, 0].cpu().detach().numpy(),
                decimals=0,
            ).astype(int)
            ...
            res += [
                (
                    ...
                    mks,
                    makespan_cp,
                    (makespan_cp - mks) / makespan_cp * 100,
                )
            ]
            print("SGS ", mks, " CP ", makespan_cp)
            print("% ", (makespan_cp - mks) / makespan_cp * 100, " % ")

    fig, ax = plt.subplots(1, figsize=(10, 10))
    ax.scatter([x[0] for x in res], [x[1] for x in res])
    ax.set_title(
        "Imprecision between infered schedule and postprocess one, function of the uncertainty range"
    )
    ax.set_xlabel(f"Delta of uncertainty on duration")
    ax.set_ylabel("Delta between sgs postpro and gnn inference.")
    fig.savefig("imprecision_noised_scenarios.png")

    fig, ax = plt.subplots(1, figsize=(10, 10))
    ax.scatter([x[0] for x in res], [x[2] for x in res])
    ax.set_title("Makespan obtained for different sampled scenarios")
    ax.set_xlabel(f"Delta of uncertainty on duration")
    fig.savefig("makespan_noised_scenarios.png")

    fig, ax = plt.subplots(1, figsize=(10, 10))
    ax.scatter([x[0] for x in res], [x[4] for x in res])
    ax.set_xlabel(f"Delta of uncertainty on duration")
    ax.set_ylabel("Overcost of CP in % (>0 is a overcost)")
    ax.set_title(
        "Overcost of CP in % (>0 is a overcost) function of the uncertainty range"
    )
    fig.savefig("gnnsgs_vs_cp_function_of_delta_of_uncertainty.png")
    res = []
    # Experiments : pop a given number of task in the problem
    for number_remove in range(1, 10):
        for k in range(20):
            some_random_picks = random.sample(
                [
                    t
                    for t in origin_rcpsp.tasks_list
                    if t not in {origin_rcpsp.source_task, origin_rcpsp.sink_task}
                ],
                number_remove,
            )
    ...
    ax.scatter([x[0] for x in res], [x[1] for x in res])
    ax.set_title(
        "Imprecision between infered schedule and postprocess one, function of task removal"
    )
    ax.set_xlabel("Number of task removal")
    ax.set_ylabel("Delta between sgs postpro and gnn inference.")
    fig.savefig("imprecision_task_removal.png")

    fig, ax = plt.subplots(1, figsize=(10, 10))
    ax.scatter([x[0] for x in res], [x[2] for x in res])
    ax.set_title("Makespan found by gnn+sgs, function of task removal")
    ax.set_xlabel("Number of task removal")
    fig.savefig("makespan_task_removal.png")

    fig, ax = plt.subplots(1, figsize=(10, 10))
    ax.scatter([x[0] for x in res], [x[4] for x in res])
    ax.set_xlabel("Number of task removal")
    ax.set_ylabel("Overcost of CP versus gnn+sgs, function of task removal")
    fig.savefig("overcost_cp_vs_gnn_task_removal.png")
    plt.show()
