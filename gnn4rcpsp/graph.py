...
class Graph:
    NODE_INPUT_DIM = 2 + 1 + 1 # first 2 are onehot(resource|task), last 2 are (#resources available, task duration)
    EDGE_FEATURE_DIM = 4 + 1 # first 4 are onehot(to_successor|to_consumer|to_predecessor|to_provider),, last is for #resources
...
#         )
            ...
            for tt in s:
                t2t.append(
                    (
                        task_ids[t],
                        task_ids[tt],
                        rcpsp_model.mode_details[t][1]["duration"] + 1e-8, # duration must be strictly positive. TODO: alternatives?
                    )
                )
        ...
        node_features = torch.tensor(
            ...
            + [[0, 1, 0, v[1]["duration"]] for v in rcpsp_model.mode_details.values()]
        )
        ...
        con = torch.stack([
            torch.stack([
                torch.stack([
                    _r2t_[r][tp] if tp != t else torch.tensor(0.)
        ...
        self._node_labels = [
            ...
            "T{}[D={}]".format(t, v[1]["duration"])
            for t, v in rcpsp_model.mode_details.items()
        ]
    ...
    def plot_graph(self):
        ...
        nx.draw(
            ...
            node_color=["lightcoral" if l[0] == "R" else "cornflowerblue" for l in self._node_labels],
            edge_color=["lightcoral" if l[0] == "C" else "cornflowerblue" for l in self._edge_labels] + ["gray"] *len(self._edge_labels),
        )
