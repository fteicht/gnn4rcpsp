...
def make_feasible_nlopt(data):
    ...
    def objective(xorig, x, dur, grad):
        ...
        for j in range(len(grad)):
            grad[j] = (1. / float(len(x))) * 2. * v[j]
        return (1. / float(len(x))) * np.dot(v, v.T)
    ...
    def resource_constraint(r, t, r2t, dur, x, grad):
        ...
        for tp in range(r2t.shape[1]):
            if tp != t and r2t[r][tp] > 0:
                grad[tp] = r2t[r][tp] * (2*x[t] - 2*x[tp] - dur[tp]) if (x[t] - x[tp])*(x[tp] + dur[tp] - x[t]) > 0 else 0.
        grad[t] = np.dot((np.array([(x[t] - x[tp])*(x[tp] + dur[tp] - x[t])
                                                     for tp in range(r2t.shape[1]) if tp != t and r2t[r][tp] > 0]) > 0).astype(int).T,
                                          np.array([(2*x[tp] - 2*x[t] + dur[tp]) * r2t[r][tp] for tp in range(r2t.shape[1]) if tp != t and r2t[r][tp] > 0]))
        return r2t[r][t] - rc[r] + np.dot((np.array([(x[t] - x[tp])*(x[tp] + dur[tp] - x[t])
                                                     for tp in range(r2t.shape[1]) if tp != t and r2t[r][tp] > 0]) > 0).astype(int).T,
    ...
    for data in data_list:
        ...
        for i in range(nb_tasks):
            opt.add_inequality_constraint(lambda x, grad: all_positive_contraint(i, x, grad), 1e-8)
        
        for t in range(t2t.shape[0]):
            for p in range(t2t.shape[1]):
                if t2t[t][p] > 0:
                    opt.add_inequality_constraint(lambda x, grad: precedence_constraint(t, p, t2t, x, grad), 1e-8)

        for r in range(r2t.shape[0]):
            for t in range(r2t.shape[1]):
                if r2t[r][t] > 0:
                    opt.add_inequality_constraint(lambda x, grad: resource_constraint(r, t, r2t, dur, x, grad), 1e-8)
        ...
        for t in range(t2t.shape[0]):
            for p in range(t2t.shape[1]):
                if t2t[t][p] > 0:
                    feasible = feasible and (precedence_constraint(t, p, t2t, xdest, grad) <= 1e-8)

        for r in range(r2t.shape[0]):
            for t in range(r2t.shape[1]):
                if r2t[r][t] > 0:
                    feasible = feasible and (resource_constraint(r, t, r2t, dur, xdest, grad) <= 1e-8)
    ...
    for data in data_list:
        ...
        if SEARCH_FOR_OPTIMALITY:
            ...
            if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                ...
                cpsat_result['optimization_abs_makespan'].append(int(solver.Value(makespan)))
            elif status == cp_model.INFEASIBLE:
...
def test(test_loader, test_list, model, device, writer):
    ...
    for batch_idx, data in enumerate(tqdm(test_loader)):
        ...
        batch_result = {
            ...
            'feasibility_rel_makespan_cor': rel_makespan['feasibility_rel_makespan_cor'],
            'feasibility_rel_makespan_ref': rel_makespan['feasibility_rel_makespan_ref'],
        ...
        if SEARCH_FOR_OPTIMALITY:
            batch_result.update({
                ...
                'optimization_rel_makespan_cor': rel_makespan['optimization_rel_makespan_cor'],
                'optimization_rel_makespan_ref': rel_makespan['optimization_rel_makespan_ref'],
                'optimization_abs_makespan': rel_makespan['optimization_abs_makespan']
            })
            
        if 'schedule' in rel_makespan:
            batch_result.update({
                'schedule': rel_makespan['schedule']
            })
        
        result_dict.update({
            'Benchmark {}'.format(batch_idx): batch_result
        })
...
if __name__ == "__main__":
    ...
    with open('../cp_solutions/inference_vs_cpsat.json', 'w') as jsonfile:
